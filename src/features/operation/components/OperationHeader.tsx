...
import { Camera, Wifi, Activity, BellOff, PauseCircle, PlayCircle, Power, Settings2, Archive, Cpu } from 'lucide-react';
...
export function OperationHeader() {
  const { activeSession, pauseSession, resumeSession, endSession, cameraMode } = useSessionStore();
  const { active } = useAlertStore();
  const clearHistory = useVitalsStore((s) => s.clearHistory);
  const streamStatus = useCameraStreamStore((s) => s.status);
  ...
  // M5.8.1: the only way out of Active Operation is an explicit, confirmed
  // End Operation -- there used to be a "Review" shortcut here that opened
  // /review while the camera and the session were both still live, which is
  // exactly the "peek at Review mid-case" surface the product flow no
  // longer has room for (Review now represents a COMPLETED operation only,
  // never the live one). Ending stops camera capture and the vitals WS
  // (both keyed off activeSession.status === 'active' / cameraMode), keeps
  // every already-persisted observation exactly as recorded, and hands off
  // to Review & Sign-off for this same session id.
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-4 h-full border-l border-slate-200 flex-shrink-0">
        ...
        {/* Camera — real state, not a toggle: reflects cameraMode + the live
            capture loop's actual status. Click jumps to Calibration, the
            only place camera mode is turned on/off or fixed. */}
        <motion.button
          onClick={() => navigate('/calibration')}
          ...
          style={
            !cameraMode
              ...
              : streamStatus === 'active' || streamStatus === 'uploading'
              ? { background: '#F0FDF4', borderColor: '#BBF7D0', color: '#16A34A' }
              : streamStatus === 'disconnected' || streamStatus === 'upload_error'
          ...
          aria-label={cameraMode ? `Camera ${streamStatus}` : 'Camera off — synthetic data'}
        >
          <Camera size={11} />
          {!cameraMode
            ? 'Synthetic'
        ...
        {/* End Operation — the only way out of Active Operation. Stops the
            camera/vitals loop, preserves everything already persisted, and
            hands off to Review & Sign-off for this session. */}
        <motion.button
          ...
          whileHover={{ scale: 1.03, boxShadow: '0 3px 12px rgba(220,38,38,0.35)' }}
          whileTap={{ scale: 0.97 }}
        >
          <Power size={11} />
          End Operation
